...
class TestMagicParsing:
    ...
    def test_non_magic_returns_false(self, kernel):
        """Test that non-magic lines return False."""
        is_magic, result = kernel._handle_magic("hello world")
        assert is_magic is False
        assert result is None

    def test_simple_magic_parsed(self, kernel):
        """Test that simple magic commands are parsed."""
        is_magic, result = kernel._handle_magic("%agent")
        assert is_magic is True
        assert result is not None

    def test_unknown_magic(self, kernel):
        """Test unknown magic command handling."""
        is_magic, result = kernel._handle_magic("%unknownmagic")
        assert is_magic is True
...
class TestAgentMagic:
    ...
    def test_agent_help(self, kernel):
    ...
    def test_agent_unknown_subcommand(self, kernel):
        """Test unknown subcommand."""
        is_magic, result = kernel._handle_magic("%agent foobar")
...
class TestMCPMagic:
    ...
    def test_mcp_list_empty(self, kernel):
        """Test listing when no MCP servers configured."""
        is_magic, result = kernel._handle_magic("%agent mcp list")
        assert "No MCP servers configured" in result

    def test_mcp_add(self, kernel):
        """Test adding an MCP server."""
        is_magic, result = kernel._handle_magic("%agent mcp add myserver mycmd --arg1")
    ...
    def test_mcp_add_update(self, kernel):
        """Test updating an existing MCP server."""
        kernel._handle_magic("%agent mcp add myserver cmd1")
        kernel._handle_magic("%agent mcp add myserver cmd2 --new-arg")
        
        assert len(kernel.state.mcp_servers) == 1
        assert kernel.state.mcp_servers[0].command == "cmd2"

    def test_mcp_list_with_servers(self, kernel):
        """Test listing configured MCP servers."""
        kernel._handle_magic("%agent mcp add server1 cmd1")
        kernel._handle_magic("%agent mcp add server2 cmd2 --arg")
    ...
    def test_mcp_remove(self, kernel):
        """Test removing an MCP server."""
        kernel._handle_magic("%agent mcp add myserver cmd")
        is_magic, result = kernel._handle_magic("%agent mcp remove myserver")
        
        assert "Removed MCP server 'myserver'" in result
        assert len(kernel.state.mcp_servers) == 0

    def test_mcp_remove_nonexistent(self, kernel):
        """Test removing a non-existent server."""
        is_magic, result = kernel._handle_magic("%agent mcp remove nosuchserver")
        assert "No MCP server named" in result

    def test_mcp_clear(self, kernel):
        """Test clearing all MCP servers."""
        kernel._handle_magic("%agent mcp add server1 cmd1")
        kernel._handle_magic("%agent mcp add server2 cmd2")
        
        is_magic, result = kernel._handle_magic("%agent mcp clear")
        assert "Removed 2 MCP server(s)" in result
        assert len(kernel.state.mcp_servers) == 0

    def test_mcp_add_missing_args(self, kernel):
        """Test mcp add with missing arguments."""
        is_magic, result = kernel._handle_magic("%agent mcp add")
        assert "Usage:" in result

        is_magic, result = kernel._handle_magic("%agent mcp add nameonly")
        assert "Usage:" in result
...
class TestSessionMagic:
    ...
    def test_session_info(self, kernel):
        ...
        is_magic, result = kernel._handle_magic("%agent session info")
        assert "Session Information" in result
        assert "Working Directory" in result

    def test_session_new(self, kernel):
        """Test creating new session."""
        cwd = os.getcwd()
        is_magic, result = kernel._handle_magic(f"%agent session new {cwd}")
        assert "Creating new session" in result
        assert kernel.state.cwd == cwd

    def test_session_new_invalid_dir(self, kernel):
        """Test new session with invalid directory."""
        is_magic, result = kernel._handle_magic("%agent session new /nonexistent/path")
        assert "Directory does not exist" in result

    def test_session_restart(self, kernel):
        """Test session restart."""
        is_magic, result = kernel._handle_magic("%agent session restart")
        assert "Restarting session" in result


class TestPermissionsMagic:
    """Test %agent permissions subcommands."""

    def test_permissions_show(self, kernel):
        """Test showing current permission mode."""
        is_magic, result = kernel._handle_magic("%agent permissions")
        assert "Permission mode: auto" in result

    def test_permissions_set_manual(self, kernel):
        """Test setting manual permission mode."""
        is_magic, result = kernel._handle_magic("%agent permissions manual")
        assert "Permission mode set to: manual" in result
        assert kernel.state.permission_mode == "manual"

    def test_permissions_set_deny(self, kernel):
        """Test setting deny permission mode."""
        is_magic, result = kernel._handle_magic("%agent permissions deny")
        assert "Permission mode set to: deny" in result
        assert kernel.state.permission_mode == "deny"

    def test_permissions_set_invalid(self, kernel):
        """Test setting invalid permission mode."""
        is_magic, result = kernel._handle_magic("%agent permissions invalid")
        assert "Unknown permission mode" in result

    def test_permissions_history_empty(self, kernel):
        """Test empty permission history."""
        is_magic, result = kernel._handle_magic("%agent permissions history")
        assert "No permission requests recorded" in result

    def test_permissions_history_with_entries(self, kernel):
...
class TestConfigMagic:
    ...
    def test_config_display(self, kernel):
        """Test config display."""
        is_magic, result = kernel._handle_magic("%agent config")
...
class TestEnvMagic:
    ...
    def test_env_display(self, kernel):
        """Test environment variable display."""
        is_magic, result = kernel._handle_magic("%agent env")
        assert "Environment Variables" in result
        assert "ACP_AGENT_COMMAND" in result

    def test_env_hides_api_key(self, kernel):
